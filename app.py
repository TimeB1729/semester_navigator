...
def _apply_kbd_action(action: str, entry: PDFEntry,
                      total_pages: int, total_pdfs: int) -> None:
    """Translate a keyboard action token into state mutations."""
    if not action:
        return
    ...
    if action == "prev_pdf":
        if st.session_state.pdf_index > 0:
            st.session_state.pdf_index -= 1
            pdf_changed = True
    elif action == "next_pdf":
        if st.session_state.pdf_index < total_pdfs - 1:
            st.session_state.pdf_index += 1
            pdf_changed = True
        ...
        idx = ZOOM_LEVELS.index(st.session_state.zoom) \
              if st.session_state.zoom in ZOOM_LEVELS else 3
        if idx < len(ZOOM_LEVELS) - 1:
            st.session_state.zoom = ZOOM_LEVELS[idx + 1]
            zoom_changed = True
        ...
        db.set_read(entry.key, not meta["is_read"])
        meta_changed = True
    ...
    if page_changed:
        db.save_page_pos(entry.key, st.session_state.page_num)

    # Rerun strictly isolated to actual changes
    if page_changed or zoom_changed or meta_changed or pdf_changed:
        st.rerun()
