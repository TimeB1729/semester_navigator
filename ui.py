...
Navigation, zoom, and keyboard shortcuts are all handled here.

Keyboard shortcuts
------------------
We inject a tiny JS bridge that uses window.parent.postMessage to push
named actions into a hidden Streamlit text_input.  app.py polls that
input each run and converts the action token into a state mutation.
This avoids fragile DOM-button queries entirely.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# KEYBOARD SHORTCUTS  (postMessage bridge)
# ══════════════════════════════════════════════════════════════════════════════

# Actions the JS can send; mapped in app.py to state mutations.
# We use a hidden text_input whose key is "_kbd_action".
# JS writes a token ("prev_pdf", "next_pdf", "prev_page", etc.)
# and then clears it 100 ms later so subsequent keystrokes register.

_KBD_JS = """
<script>
(function() {
  if (window.__kbdAttached) return;
  window.__kbdAttached = true;

  function sendAction(action) {
    // Direct target via unique placeholder bypasses fragile div wrapper structures
    const inp = window.parent.document.querySelector('input[placeholder="__kbd_sentinel__"]');
    if (!inp) return;

    const nativeSet = Object.getOwnPropertyDescriptor(
      window.HTMLInputElement.prototype, 'value').set;
    nativeSet.call(inp, action);
    inp.dispatchEvent(new Event('input', { bubbles: true }));
    
    // Rapid clear on next tick ensures back-to-back keystrokes aren't swallowed
    setTimeout(() => {
      nativeSet.call(inp, '');
      inp.dispatchEvent(new Event('input', { bubbles: true }));
    }, 20);
  ...
  window.parent.document.addEventListener('keydown', function(e) {
    ...
    if (active) {
      ...
      // Safety guard: Never hijack keys while typing in form fields
      if (tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT' || active.isContentEditable) {
        return;
      }
    }

    // Handle Notes focus shortcut completely in the DOM
    if (e.key === 'n' || e.key === 'N') {
      e.preventDefault();
      const ta = Array.from(window.parent.document.querySelectorAll('textarea')).find(el => {
        const label = window.parent.document.querySelector(`label[for="${el.id}"]`);
        return (label && label.innerText.includes('Notes')) || el.placeholder.includes('notes');
      });
      if (ta) ta.focus();
      return;
    }
...
def inject_keyboard_shortcuts() -> None:
    """
    Inject the JS keyboard bridge.  Call once per render in the viewer column.
    The hidden sentinel input is rendered separately by render_kbd_input().
    """
    st.components.v1.html(_KBD_JS, height=0)


def render_kbd_input() -> str:
    """
    Render the hidden sentinel text_input that receives JS keystrokes.
    Returns the current action token (or empty string).
    Place this ONCE at the top of app.py, before the viewer column.
    """
    action = st.text_input(
        "kbd",
        key="_kbd_action",
        placeholder="__kbd_sentinel__",
        label_visibility="collapsed",
    )
    # Hide it with CSS
    st.markdown(
        """
        <style>
        div[data-testid="stTextInput"]:has(input[placeholder="__kbd_sentinel__"]) {
